...
const ScriptPage = () => {
	...
	useEffect(() => {
		...
		const persistedLanguage = settingsMap.get("selectedLanguage");
		const persistedShowTranslated = settingsMap.get("showTranslated");

		if (persistedLanguage) {
			setSelectedLanguage(persistedLanguage);
		} else {
			settingsMap.set("selectedLanguage", "en");
		}

		if (typeof persistedShowTranslated === "boolean") {
			setShowTranslated(persistedShowTranslated);
		} else {
			settingsMap.set("showTranslated", false);
		}

		const handleSettingsChange = () => {
			const nextLanguage = settingsMap.get("selectedLanguage");
			const nextShowTranslated = settingsMap.get("showTranslated");
			...
			if (
				typeof nextShowTranslated === "boolean" &&
				nextShowTranslated !== showTranslated
			) {
				setShowTranslated(nextShowTranslated);
			}
		};

		settingsMap.observe(handleSettingsChange);
		return () => settingsMap.unobserve(handleSettingsChange);
	}, [provider, selectedLanguage, showTranslated]);

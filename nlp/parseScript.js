...
function splitSentences(text) {
  return text.split(/(?<=[.!?])\s+/).map(t => cleanText(t)).filter(Boolean);
}
...
function parseScriptLinesAndScenes(lines) {
  ...
  const sceneStartRegex = /^(INT\.|EXT\.|EST\.|INT\/EXT\.|I\/E\.|INT-EXT\.|EXT-INT\.)/i;
  const sceneEndRegex = /(\.|--|:)$/;

  function flushBuffer() {
    if (buffer.length > 0) {
      const textBlock = cleanText(buffer.join(' '));
      if (bufferType === 'content:dialog' || bufferType === 'content:narration') {
  ...
  lines.forEach((raw, i) => {
    ...
    if ((/^[A-Z0-9 \-]+:$/.test(text) && text === text.toUpperCase()) || /^\d+\.$/.test(text)) {
      flushBuffer();
    ...
    if (!inDialogBlock && /^[A-Z0-9 '\-]+$/.test(text) && text === text.toUpperCase()) {
      flushBuffer();
